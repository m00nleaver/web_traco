...
@WebServlet("/minitour/jeju/insert/reserve/minirez.do")
public class insertRez extends HttpServlet {
	@Override
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		System.out.println("tblRez(주문예약) insert");
		...
		dto.setBankm_name(bankm_name);
		
		
		MiniPackageDAO dao = new MiniPackageDAO();
		
		int result = dao.insertRez(dto);
		...
		// { "result": 1 }
		writer.printf("{ \"result2\": %d }", result);
