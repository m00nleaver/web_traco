...
<body>
	...
	<main class="main">
		...
		<section class="content">
			...
			<form method="POST" action="/traco/admin/addcarok.do">
				<table class="table table-bordered addbox">
					...
					<tr>
						<td>자동차 종류</td>
						<td colspan="3"><c:forEach items="${klist}" var="rdto">
								<input type="radio" name="kind" value="${rdto.kindseq}"
									class="type">${rdto.kindname} 
						
					</c:forEach></td>
						<td style="background-color: #CCC;">탑승인원</td>
						<td><input type="text" name="seatnum" class="seatnum" required></td>

					</tr>
					<tr>
						<td>자동차 여부</td>
						<td><input type="radio" name="status" value="대여가능"
							class="status" checked>대여가능 <input type="radio" name="status"
							value="대여종료" class="status">대여종료</td>
