...
<body>
	...
	<main class="main">
		...
		<section class="content">
			...
			<form method="POST" action="/traco/admin/beditok.do">
				<table class="table table-bordered addbox">
					
					<tr>
						<td style="width:100px;">배너이름</td>
						<td><input type="text" name="name" value="${pdto.bname}"></td>
					</tr>
					<tr>
						<td>배너이미지</td>
						<td style=""><input type="file" name="file" style="width:180px; margin-left:100px;" value="${pdto.bfile}">기존이미지:${pdto.bfile}</td>
					</tr>
					<tr>
						<td>링크URL</td>
						<td><input type="text" name="url" value="${pdto.burl}"></td>
					</tr>
					<tr>
						<td>배너카테고리</td>
						<td>
							<select name="ctg" style="width:100px; height:30px; font-size:20px;">
								<option><c:if test="${pdto.bctg ==1}">인기</c:if><c:if test="${pdto.bctg ==2}">추천</c:if><c:if test="${pdto.bctg ==3}">이벤트</c:if></option>
				...
				<input type="hidden" value="${pdto.bseq}" name="bseq">
			</form>
