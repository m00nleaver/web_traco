...
<button type="button" id="search" class="btn btn-secondary btn-lg">검색</button>
		...
		<div style="width:1000px; margin:0px auto;">
			...
			<span class="h1 btn btn-primary" onclick="location.href='/traco/minitour/jeju/minitotalview.do';" class="h2" style="float:right;">전체상품 보기</span>
	...
	<script>
		...
		$(".big").click(() => {
			...
			$.ajax({
				type: 'GET',
				url: '/traco/minitour/miniarea.do',
		...
		$("#search").click(() => {
			...
			$.ajax({
				type: 'GET',
				url: '/traco/minitour/miniareapk.do',
				  ...
				  new Set([...taglist]).forEach((item)=>{
				   ...
				   if (list.length > 4){
					   ...
					   list.forEach((item) => {
							 ...
							 $('#recom-row').append(`<div class="col-md-3"><div class="card"><img src="/traco/asset/images/minithumb/\${item.thumbseq}.jpg" class="card-img-top" alt="..."><div class="card-body"><h5 class="h4 card-title" name="title" style="height: 50px;">\${item.name}</h5><span class="h4 card-text">\${item.adultprice}원</span><button style="margin-left : 20px;" type="submit" class="btn btn-primary detail" onclick="location.href='/traco/minitour/jeju/miniview.do?miniSeq=\${item.seq}';">상세보기</button></div></div></div>`);
					 ...
					 list.forEach((item) => {
						 ...
						 $('#hash-row').append(`<div class="col-md-3 \${item.tag} tagitems"><div class="card"><img src="/traco/asset/images/minithumb/\${item.thumbseq}.jpg" class="card-img-top" alt="..."><div class="card-body"><h5 class="h4 card-title" name="title" style="height: 50px;">\${item.name}</h5><span class="h4 card-text">\${item.adultprice}원</span><button style="margin-left : 20px;" type="submit" class="btn btn-primary detail" onclick="location.href='/traco/minitour/jeju/miniview.do?miniSeq=\${item.seq}';">상세보기</button></div></div></div>`);
