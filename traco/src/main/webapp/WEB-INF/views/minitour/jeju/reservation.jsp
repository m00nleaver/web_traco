	...
	<main class="main">
		...
		<section class="content">
			...
			<div id="bankdiv">
				...
				<div style="padding-left:70px;"class="h2">입금은행</div>
				<div>
				...
				<div style="position : relative; right : -70px; top : 50px;">
	...
	<script>
		...
		$('#is-member').click(()=>{
			...
			function memberInfo(){
					$.ajax({
					type: 'GET',
					url: '/traco/minitour/jeju/minimember.do',
			...
			$("select[id='age']").change(()=>{
				...
				$( "select[id='age'] option:selected" ).each(function() {
					 ...
					 if ($(this).val() >=12) {
					 price += parseInt(`${itemdto.MINIPM_PRICE1}`);
					 adult += 1;
				 	} 
					else if ($(this).val() >=2) {
					 price += parseInt(`${itemdto.MINIPM_PRICE2}`);
					 kid += 1;
				 	}
					else {
						 price += parseInt(`${itemdto.MINIPM_PRICE3}`);
						 toddler += 1;
					 	}
				 	});
			...
			$("#reserve input[name='pkgpm_seq']").val(`${miniSeq}`);
		...
		$("#reserveBtn").click(  ()=>{
			...
			$.ajax({
				type: 'POST',
				url: '/traco/minitour/jeju/insert/reserve/minirez.do',
				...
				success: function(result) { 
					...
					count+= res;				
						...
						$.ajax({
							type: 'POST',
							url: '/traco/minitour/jeju/insert/reserve/minipackager.do',
							data: $('#reserve').serialize(),
							success: function(result) { 
